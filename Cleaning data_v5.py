...
df1 = pd.read_csv('https://raw.githubusercontent.com/Andrea-Giuliani/Python-Project/master/data/jobs_1_26032020.csv')
df2 = pd.read_csv('https://raw.githubusercontent.com/Andrea-Giuliani/Python-Project/master/data/jobs_2_26032020.csv')
...
class TestNaN(unittest.TestCase):
    ...
    def test_NaN(self):
        ...
        Test the NaN
        '''
        rating = list[1, 2, 3, 'NOT_FOUND', 4, 'NOT_FOUND', 6]
        rating2 = list[1, 2, 3, NaN, 4, NaN, 6]
        self.assertEqual(mark_nan_values)
        
if __name__ == '__main__':
    unittest.main(exit=False) 


# We count the number of NaN values in each column of the data frame.
...
class TestDrop(unittest.TestCase):
    ...
    def test_drop(self):
        """ ADD DESCRIPTION """
        df = ['Location', 'Rating', 'City']
        df_drop = drop_col(df)
        self.assertEqual(drop_col, 'Rating', 'City')
...
df_merged_types.to_csv("data_clean_v1.csv", index=True, encoding='utf8')


###############

data_clean = pd.read_csv('https://raw.githubusercontent.com/Andrea-Giuliani/Python-Project/master/data/df_cleanv2(merge).csv')
...
def group_by_company(data_clean):
    ...
    cat_data_clean = data_clean.groupby('Company_name') \
                .agg({'Job_title':pd.Series.nunique,
                      'Unnamed: 0.1': pd.Series.nunique})\
                .rename(columns = {'Job_title':'num_job_title',
                                   'Unnamed: 0.1': 'num_job_posts'})\
...
class TestProcessing (unittest.TestCase):
    ...
    def test_processing(self):
        ...
        '''
        pre_process = "For Data sciEntits. is important worK tema"
        post_process = basic_pre_processing(pre_process)
        self.assertEqual(post_process, 'data scientists work team')
